...
CLANG ?= clang
LLVM_VERSION ?= $(shell $(CLANG) -dumpversion 2>/dev/null | cut -d. -f1)
LLVM_STRIP ?= $(or $(shell command -v llvm-strip 2>/dev/null),$(shell command -v llvm-strip-$(LLVM_VERSION) 2>/dev/null),llvm-strip)
